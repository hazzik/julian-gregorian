...
describe('Test calendar conversion', () => {
    ...
    it('Convert from gregorian calendar date to julian calendar date', (done) => {
        ...
        CalendarConverter.fromGregorianToJulian({year: 199, month: 12,day:31}).should.be.eql({year:  200, month: 1, day: 1});
        CalendarConverter.fromGregorianToJulian({year: 200, month: 2, day:28}).should.be.eql({year:  200, month: 2, day:29});
        done();
    });
    ...
    it('Convert from julian calendar date to gregorian calendar date', (done) => {
        ...
        CalendarConverter.fromJulianToGregorian({year: 200, month: 2, day:29}).should.be.eql({ year:  200, month: 2, day: 28 });
        CalendarConverter.fromJulianToGregorian({year: 200, month: 1, day: 1}).should.be.eql({ year:  199, month:12, day: 31 });
        done();
    });
});
